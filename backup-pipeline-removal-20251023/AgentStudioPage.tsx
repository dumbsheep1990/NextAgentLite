...
// 统一的“智能体工作室”页面：左侧会话、中间对话测试、右侧配置栏
  ...
  // Hook配置（支持Pipeline模式和直接选择模式）
  const [pipelineMode, setPipelineMode] = useState<'pipeline' | 'custom'>('pipeline');
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setAvailablePipelines(pipelineList);
        ...
        if (templateId) {
          ...
          try {
            ...
            if (agentDetail.custom_config) {
              ...
              // 加载Hook Pipeline配置（支持Pipeline模式和自定义模式）
              if (agentDetail.default_pipeline_id) {
                // Pipeline模式：使用预定义的Hook Pipeline
                setPipelineMode('pipeline');
                setSelectedPipelineId(agentDetail.default_pipeline_id);
                console.log('[AgentStudio] 加载Pipeline配置:', agentDetail.default_pipeline_id);
              } else if (customCfg.hooks) {
                // 自定义模式：直接选择Pre-hooks和Post-hooks
                ...
                console.log('[AgentStudio] 加载自定义Hook配置:', hooksCfg);
              } else if (customCfg.hook_pipeline) {
                // 旧格式：向后兼容（暂不加载，因为新UI不支持旧版pipeline格式）
                console.warn('[AgentStudio] 检测到旧版hook_pipeline配置，已忽略。请使用新的配置方式。');
              }
            }
  ...
  const handleSave = async () => {
    try {
      ...
      const draft = { templateId, createdAt: Date.now(), req: {
        ...
        // Hook Pipeline配置（根据模式选择保存方式）
        default_pipeline_id: pipelineMode === 'pipeline' ? selectedPipelineId : null,
        custom_config: {
          ...
          // Hook配置（自定义模式时保存直接选择的hooks）
          hooks: pipelineMode === 'custom' ? {
