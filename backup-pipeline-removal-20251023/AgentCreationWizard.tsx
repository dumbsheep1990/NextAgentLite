...
const AgentCreationWizard: React.FC<AgentCreationWizardProps> = ({
  ...
  const loadInitialData = async () => {
    ...
    try {
      const tasks = [
        ...
        fetch('/api/v1/hook-pipelines/available/hooks').then(r => r.json())  // 4: hooks
      ];
      const results = await Promise.allSettled(tasks);

      const [tplRes, colRes, toolRes, modelRes, hooksRes] = results as const;
      ...
      if (hooksRes.status === 'fulfilled') {
        const hookData = (hooksRes.value as any)?.hooks || [];
        setHooks(hookData);
      } else {
        setHooks([]);
      }
      ...
      // 汇总错误但不阻断页面，指出具体失败项
      const parts = ['模板', '知识库', '工具', '模型', 'Hooks'];
      const failed: string[] = [];
    ...
    return (
      <div className="space-y-4">
        ...
        <Tabs defaultActiveKey="builtin" items={[
          {
            key: 'builtin',
            label: <span>内置工具 <Tag color="blue">{builtin.length}</Tag></span>,
            children: <ToolList items={builtin} />
          },
          {
            key: 'mcp',
            label: <span>MCP 服务 <Tag color="blue">{mcp.length}</Tag></span>,
            children: <ToolList items={mcp} />
          },
          {
            key: 'api',
            label: <span>API 配置 <Tag color="blue">{api.length}</Tag></span>,
            children: <ToolList items={api} />
          },
          {
            key: 'custom',
            label: <span>自定义工具 <Tag color="blue">{custom.length}</Tag></span>,
            children: <ToolList items={custom} />
          }
        ]} />
      </div>
    );
  ...
  // Hook选择相关辅助函数
  const togglePreHook = (hook_id: string) => {
    const existing = formData.pre_hooks.find(h => h.hook_id === hook_id);
    if (existing) {
      // 已选中，移除
      updateFormData({ pre_hooks: formData.pre_hooks.filter(h => h.hook_id !== hook_id) });
    } else {
      // 未选中，添加
      const newOrder = formData.pre_hooks.length > 0
        ? Math.max(...formData.pre_hooks.map(h => h.order)) + 1
        : 1;
      updateFormData({
        pre_hooks: [...formData.pre_hooks, { hook_id, enabled: true, order: newOrder }]
      });
    }
  };

  const togglePostHook = (hook_id: string) => {
    const existing = formData.post_hooks.find(h => h.hook_id === hook_id);
    if (existing) {
      // 已选中，移除
      updateFormData({ post_hooks: formData.post_hooks.filter(h => h.hook_id !== hook_id) });
    } else {
      // 未选中，添加
      const newOrder = formData.post_hooks.length > 0
        ? Math.max(...formData.post_hooks.map(h => h.order)) + 1
        : 1;
      updateFormData({
        post_hooks: [...formData.post_hooks, { hook_id, enabled: true, order: newOrder }]
      });
    }
  };

  const moveHook = (hookType: 'pre' | 'post', hook_id: string, direction: 'up' | 'down') => {
    const hookList = hookType === 'pre' ? formData.pre_hooks : formData.post_hooks;
    const currentIndex = hookList.findIndex(h => h.hook_id === hook_id);
    ...
    // 重新分配order
    newList.forEach((h, idx) => {
      h.order = idx + 1;
    });

    if (hookType === 'pre') {
      updateFormData({ pre_hooks: newList });
    } else {
      updateFormData({ post_hooks: newList });
    }
  };

  // 渲染步骤4: Hook配置
  const renderStep4 = () => {
    const preHooks = hooks.filter(h => h.hook_type === 'pre');
    const postHooks = hooks.filter(h => h.hook_type === 'post');

    const selectedPreHooks = formData.pre_hooks
      .map(config => ({ ...hooks.find(h => h.hook_id === config.hook_id), ...config }))
      .filter(h => h.hook_id);

    const selectedPostHooks = formData.post_hooks
      .map(config => ({ ...hooks.find(h => h.hook_id === config.hook_id), ...config }))
      .filter(h => h.hook_id);
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <Text type="secondary">配置智能体执行前后的拦截器（可选）</Text>
        </div>

        {/* Pre-Hooks */}
        <div>
          <Title level={5}>Pre-Hooks（前置拦截器）</Title>
          <Text type="secondary" className="block mb-3">在核心逻辑执行前运行，可用于输入预处理、验证等</Text>

          <div className="mb-4">
            <Text strong>选择 Pre-Hooks:</Text>
            <div className="mt-2 space-y-2">
              {preHooks.map(hook => (
                <Card
                  key={hook.hook_id}
                  size="small"
                  className={`cursor-pointer ${formData.pre_hooks.some(h => h.hook_id === hook.hook_id) ? 'border-blue-500 bg-blue-50' : ''}`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <Checkbox
                        checked={formData.pre_hooks.some(h => h.hook_id === hook.hook_id)}
                        onChange={() => togglePreHook(hook.hook_id)}
                      />
                      <div>
                        <div className="flex items-center space-x-2">
                          <Text strong>{hook.name}</Text>
                          {hook.is_custom && <Tag color="orange">自定义</Tag>}
                          {hook.category && <Tag color="blue">{hook.category}</Tag>}
                        </div>
                        {hook.description && (
                          <Text type="secondary" className="text-xs">{hook.description}</Text>
                        )}
                      </div>
                    </div>
                  </div>
              ...
              {preHooks.length === 0 && (
                <Alert type="info" message="暂无可用的Pre-Hooks，可以在工具-Hook管理中创建" />
              )}
            </div>
          </div>

          {selectedPreHooks.length > 0 && (
            <div>
              <Text strong>已选 Pre-Hooks 顺序:</Text>
              <div className="mt-2 space-y-2">
                {selectedPreHooks.map((hook, index) => (
                  <Card key={hook.hook_id} size="small" className="bg-blue-50">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <div className="flex items-center space-x-2">
                          <span className="text-sm font-bold text-blue-600">#{hook.order}</span>
                          <Text strong>{hook.name}</Text>
                          <Switch
                            size="small"
                            checked={hook.enabled}
                            onChange={(checked) => {
                              const newList = formData.pre_hooks.map(h =>
                                h.hook_id === hook.hook_id ? { ...h, enabled: checked } : h
                              );
                              updateFormData({ pre_hooks: newList });
                            }}
                          />
                      ...
                      <Space>
                        <Button
                          size="small"
                          disabled={index === 0}
                          onClick={() => moveHook('pre', hook.hook_id, 'up')}
                        >
                          ↑
                        </Button>
                        <Button
                          size="small"
                          disabled={index === selectedPreHooks.length - 1}
                          onClick={() => moveHook('pre', hook.hook_id, 'down')}
                        >
                          ↓
                        </Button>
                      </Space>
        ...
        {/* Post-Hooks */}
        <div>
          <Title level={5}>Post-Hooks（后置拦截器）</Title>
          <Text type="secondary" className="block mb-3">在核心逻辑执行后运行，可用于输出格式化、结果处理等</Text>

          <div className="mb-4">
            <Text strong>选择 Post-Hooks:</Text>
            <div className="mt-2 space-y-2">
              {postHooks.map(hook => (
                <Card
                  key={hook.hook_id}
                  size="small"
                  className={`cursor-pointer ${formData.post_hooks.some(h => h.hook_id === hook.hook_id) ? 'border-green-500 bg-green-50' : ''}`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <Checkbox
                        checked={formData.post_hooks.some(h => h.hook_id === hook.hook_id)}
                        onChange={() => togglePostHook(hook.hook_id)}
                      />
                      <div>
                        <div className="flex items-center space-x-2">
                          <Text strong>{hook.name}</Text>
                          {hook.is_custom && <Tag color="orange">自定义</Tag>}
                          {hook.category && <Tag color="green">{hook.category}</Tag>}
                        </div>
                        {hook.description && (
                          <Text type="secondary" className="text-xs">{hook.description}</Text>
                        )}
              ...
              {postHooks.length === 0 && (
                <Alert type="info" message="暂无可用的Post-Hooks，可以在工具-Hook管理中创建" />
              )}
            </div>
          </div>

          {selectedPostHooks.length > 0 && (
            <div>
              <Text strong>已选 Post-Hooks 顺序:</Text>
              <div className="mt-2 space-y-2">
                {selectedPostHooks.map((hook, index) => (
                  <Card key={hook.hook_id} size="small" className="bg-green-50">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <div className="flex items-center space-x-2">
                          <span className="text-sm font-bold text-green-600">#{hook.order}</span>
                          <Text strong>{hook.name}</Text>
                          <Switch
                            size="small"
                            checked={hook.enabled}
                            onChange={(checked) => {
                              const newList = formData.post_hooks.map(h =>
                                h.hook_id === hook.hook_id ? { ...h, enabled: checked } : h
                              );
                              updateFormData({ post_hooks: newList });
                            }}
                      ...
                      <Space>
                        <Button
                          size="small"
                          disabled={index === 0}
                          onClick={() => moveHook('post', hook.hook_id, 'up')}
                        >
                          ↑
                        </Button>
                        <Button
                          size="small"
                          disabled={index === selectedPostHooks.length - 1}
                          onClick={() => moveHook('post', hook.hook_id, 'down')}
                        >
                          ↓
                        </Button>
                      </Space>
