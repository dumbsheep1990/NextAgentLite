...
def _add_react_tool_instructions(agent) -> None:
    ...
    for tool in agent.tools:
        ...
        # 方式1: 直接从工具对象获取name和description属性
...
async def _add_hook_integration(
    ...
    """为workflow添加Hook Pipeline集成

    此函数提取自step_prepare，用于在多个workflow中复用Hook集成逻辑。
    执行Pre-hooks并将结果注入到workflow上下文。
    ...
    try:
        ...
        if pipeline_id:
            ...
            if not hook_pipeline:
                logger.warning(f"[WF][{run_id}] Pipeline加载失败: {pipeline_id}，使用默认Pipeline")
        ...
        try:
            ...
            pre_results = await hook_pipeline.execute_pre_hooks(
                run_input,
                session,
                user_id=ctx.inputs.get('user_id'),
            ...
            ctx.vars['pre_hook_results'] = pre_results
    ...
    try:
        ...
        if hook_pipeline:
            ...
            try:
                ...
                ctx.vars['query_features'] = run_input.context.get('query_features')
                ctx.vars['hook_pipeline'] = hook_pipeline
                ctx.vars['pre_hook_results'] = pre_results  # 保存执行结果
