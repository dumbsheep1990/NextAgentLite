...
async def team_query(
    ...
    try:
        ...
        logger.info(f"[TEAM API] 收到Team查询请求: {team_name}, 查询: {query[:50]}..., 知识检索模式: {knowledge_retrieval_mode}")
        
        if stream:
            # 流式响应 - 不在服务端保存消息，由前端负责保存
            return StreamingResponse(
                _stream_team_response(team_name, query, session_id, enable_monitoring, knowledge_retrieval_mode),
            ...
            try:
                ...
                await conversation_repo.save_team_conversation(
                    ...
                    sources=None,
                    knowledge_sources=response.monitoring_data.get('knowledge_sources', []) if response.monitoring_data else [],
                    thinking=None,
...
async def _stream_team_response(team_name: str, query: str, session_id: str, enable_monitoring: bool, knowledge_retrieval_mode: str = "all"):
        ...
        async for event in enhanced_team_service.execute_team_query_stream(
            ...
            if isinstance(event, dict):
                if event_type == "content":
                    ...
                    # 🔥 关键修复：确保memberCalls数据被正确记录和转发
                    member_calls_in_analysis = analysis_data.get("member_calls", [])
                    if member_calls_in_analysis:
                        logger.info(f"[TEAM STREAM] 收集到team分析数据，包含 {len(member_calls_in_analysis)} 个memberCalls")
                        # 确保事件数据包含memberCalls信息
                        if "member_calls" not in event.get("data", {}):
                            event["data"]["member_calls"] = member_calls_in_analysis
                    else:
                        logger.warning(f"[TEAM STREAM] team_analysis事件中没有memberCalls数据")
                    
                    # 🔥 新增：收集teamDecisions数据
                    team_decisions_in_analysis = analysis_data.get("team_decisions", [])
                    if team_decisions_in_analysis:
                        logger.info(f"[TEAM STREAM] 收集到team决策数据，包含 {len(team_decisions_in_analysis)} 个teamDecisions")
                        # 将teamDecisions添加到全局收集器中
                        team_decisions.extend(team_decisions_in_analysis)
                        # 同时也添加到team_info中
                        if "team_decisions" not in team_info:
                            team_info["team_decisions"] = []
                        team_info["team_decisions"].extend(team_decisions_in_analysis)
                        # 确保事件数据包含teamDecisions信息  
                        if "team_decisions" not in event.get("data", {}):
                            event["data"]["team_decisions"] = team_decisions_in_analysis
                    else:
                        logger.warning(f"[TEAM STREAM] team_analysis事件中没有teamDecisions数据")
        ...
        if session_id:
            try:
                ...
                async with get_async_session() as db_session:
                    ...
                    final_team_info = team_info if team_info else {"status": "completed", "final_update": True}
                    
                    # 添加execution_id到team_info中
                    final_team_info["executionId"] = execution_id
                    final_team_info["teamName"] = team_name
                    final_team_info["teamMode"] = "coordinate"
