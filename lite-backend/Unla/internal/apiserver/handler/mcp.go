...
func (h *MCP) checkTenantPermission(c *gin.Context, tenantName string, cfg *config.MCPConfig) (*database.Tenant, error) {
    ...
    // If claims exist and role is admin, allow fast path (skip user lookup)
    if cClaims, ok := c.Get("claims"); ok {
        if jc, ok2 := cClaims.(*jwt.Claims); ok2 {
            if strings.ToLower(jc.Role) == "admin" {
                ...
                if !strings.HasPrefix(tenantPrefix, "/") { tenantPrefix = "/" + tenantPrefix }
                ...
                for _, router := range cfg.Routers {
                    ...
                    if rp == tenantPrefix { continue }
                    if !strings.HasPrefix(rp, tenantPrefix+"/") {
                        h.logger.Warn("router prefix validation failed",
                            zap.String("router_prefix", rp),
    ...
    // Embedded mode: fully trust local calls, skip JWT requirement
    if isEmbeddedMode() {
        tenant, err := h.db.GetTenantByName(c.Request.Context(), tenantName)
        if err != nil {
            h.logger.Warn("tenant not found",
        ...
        // still validate router prefixes against tenant prefix
        tenantPrefix := tenant.Prefix
    ...
    // Get user authentication information (non-embedded)
    ...
    // Get user information (non-embedded or non-admin)
...
func isEmbeddedMode() bool {
    v := os.Getenv("APISERVER_EMBEDDED_MODE")
...
func (h *MCP) HandleMCPServerUpdate(c *gin.Context) {
    ...
    // After successful update, trigger 9050 tools catalog sync (best-effort)
    go triggerToolsCatalogSync(h.logger)
...
func (h *MCP) HandleListMCPServers(c *gin.Context) {
    ...
    if exists {
        jwtClaims, _ = claims.(*jwt.Claims)
    }
    isAdmin := jwtClaims != nil && strings.ToLower(jwtClaims.Role) == "admin"

    var user *database.User
    if !isAdmin {
        if jwtClaims == nil {
            // no claims and not admin: in embedded mode allow as admin
            if isEmbeddedMode() {
                isAdmin = true
            } else {
                h.logger.Warn("missing JWT claims in context")
            ...
            user, err = h.db.GetUserByUsername(c.Request.Context(), jwtClaims.Username)
            if err != nil {
                // Treat as admin in embedded mode to avoid blocking
                if isEmbeddedMode() {
                    isAdmin = true
                } else {
                    h.logger.Error("failed to get user info",
    ...
    if !isAdmin && user != nil && user.Role != database.RoleAdmin && tenantID > 0 {
		userTenants, err := h.db.GetUserTenants(c.Request.Context(), user.ID)
		if err != nil {
			h.logger.Error("failed to get user tenants",
				zap.Uint("user_id", user.ID),
    ...
    } else if !isAdmin && user != nil && user.Role != database.RoleAdmin {
		userTenants, err := h.db.GetUserTenants(c.Request.Context(), user.ID)
		if err != nil {
			h.logger.Error("failed to get user tenants",
				zap.Uint("user_id", user.ID),
...
func (h *MCP) HandleMCPServerCreate(c *gin.Context) {
    ...
    // After successful create, trigger 9050 tools catalog sync (best-effort)
    go triggerToolsCatalogSync(h.logger)
...
func (h *MCP) HandleMCPServerDelete(c *gin.Context) {
    ...
    // After successful delete, trigger 9050 tools catalog sync (best-effort)
    go triggerToolsCatalogSync(h.logger)
	i18n.Success(i18n.SuccessMCPServerDeleted).With("status", "success").Send(c)
}

func (h *MCP) HandleMCPServerSync(c *gin.Context) {
    // In embedded/trusted mode, allow sync without user lookup
    isAdmin := false
    if claims, ok := c.Get("claims"); ok {
        if jc, ok2 := claims.(*jwt.Claims); ok2 && strings.ToLower(jc.Role) == "admin" {
            isAdmin = true
        }
    }
    ...
    if !isAdmin {
        // Fallback to strict check only when not embedded and not admin claims
        claims, exists := c.Get("claims")
        if !exists {
            h.logger.Warn("missing JWT claims in context")
            i18n.RespondWithError(c, i18n.ErrUnauthorized)
...
func (h *MCP) HandleSetActiveVersion(c *gin.Context) {
    ...
    // After switching active version, trigger 9050 tools catalog sync (best-effort)
    go triggerToolsCatalogSync(h.logger)
	i18n.Success(i18n.SuccessMCPServerUpdated).With("status", "success").Send(c)
}

// triggerToolsCatalogSync calls 9050 /admin/sync-tools-catalog to refresh tool catalog
func triggerToolsCatalogSync(logger *zap.Logger) {
    base := os.Getenv("LLM_GATEWAY_URL")
    if base == "" { base = "http://127.0.0.1:9050" }
    url := strings.TrimRight(base, "/") + "/admin/sync-tools-catalog"
    req, err := http.NewRequest(http.MethodPost, url, bytes.NewReader([]byte("{}")))
    if err != nil { logger.Warn("build sync-tools-catalog request failed", zap.Error(err)); return }
    req.Header.Set("Content-Type", "application/json")
    client := &http.Client{ Timeout: 5 * time.Second }
    resp, err := client.Do(req)
    if err != nil { logger.Warn("sync-tools-catalog failed", zap.Error(err)); return }
    defer resp.Body.Close()
    if resp.StatusCode >= 300 {
        logger.Warn("sync-tools-catalog non-2xx", zap.Int("status", resp.StatusCode))
    } else {
        logger.Info("sync-tools-catalog ok")
    }
}
