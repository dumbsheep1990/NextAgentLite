...
// GetMCPServerName returns the MCP server name for a given prefix, if any
func (s *State) GetMCPServerName(prefix string) string {
    runtime, ok := s.runtime[uriPrefix(prefix)]
    if !ok || runtime.mcpServer == nil { return "" }
    return runtime.mcpServer.Name
}
