...
type (
	...
	Server struct {
		...
		// apiserver base for MCP registry registration
...
func NewServer(logger *zap.Logger, port int, store storage.Store, sessionStore session.Store, a auth.Auth, forwardConfig config.ForwardConfig) (*Server, error) {
	s := &Server{
		...
		apiserverBase:   getEnvDefault("UNLA_APISERVER_BASE", "http://localhost:5234"),
	}
...
func (s *Server) RegisterRoutes(ctx context.Context) error {
	...
	// Try to register MCP instances to apiserver registry (best-effort)
	go s.registerMCPInstances(ctx)

	return nil
}

// registerMCPInstances posts the running (or configured) MCP instances into apiserver registry for discovery
func (s *Server) registerMCPInstances(ctx context.Context) {
    st := s.state
    if st == nil { return }
    client := &http.Client{ Timeout: 5 * time.Second }
    for prefix, transport := range st.GetTransports() {
        name := st.GetMCPServerName(prefix)
        if name == "" {
            name = strings.TrimPrefix(prefix, "/")
        }
        transportType := string(st.GetProtoType(prefix))
        endpoint := fmt.Sprintf("http://localhost:%d%s", s.port, prefix)
        status := "stopped"
        if transport != nil && transport.IsRunning() { status = "running" }
        ...
        url := s.apiserverBase + "/api/mcp/registry/register"
        req, _ := http.NewRequestWithContext(ctx, http.MethodPost, url, bytes.NewReader(b))
        req.Header.Set("Content-Type", "application/json")
        if resp, err := client.Do(req); err != nil {
            s.logger.Debug("register mcp instance failed", zap.Error(err), zap.String("url", url))
        } else {
            _ = resp.Body.Close()
        }
    }
