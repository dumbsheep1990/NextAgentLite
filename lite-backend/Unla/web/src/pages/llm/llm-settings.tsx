...
const LLMSettings: React.FC = () => {
  ...
  const [defaultChatInfo, setDefaultChatInfo] = useState<any>(null);

  // 从本地 providers 推断默认模型所属厂商（用于兜底显示图标）
  const inferredDefaultProviderId = useMemo(() => {
    if (!defaultChatModel) return '';
    for (const p of providers as any[]) {
      if (Array.isArray(p?.models) && p.models.some((m: any) => m.id === defaultChatModel)) {
        return String(p.id).toLowerCase();
      }
    }
    return '';
  }, [providers, defaultChatModel]);
  ...
  // 加载网关默认 chat 模型（来自 9050）
  useEffect(() => {
    (async () => {
      try {
        const d = await llmGatewayApi.getDefaults();
        setDefaultChatModel(d?.default_model || '');
        setDefaultChatInfo(d?.default_model_info || null);
      } catch (err: any) {
        // 不打断页面，但打印详细错误，便于定位“后台无日志”的问题
        // 这里的后台通常是 9050，而不是 5234
        // 如果网关未启动或 CORS/URL 配置错误，这里会暴露出来
        // eslint-disable-next-line no-console
        console.error('加载默认模型失败(getDefaults):', err?.response?.status, err?.response?.data || err?.message);
      }
    })();
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 将当前全量 providers 同步到统一模型网关（9050），便于后端使用
      try {
        const payload = {
          default_model: defaultChatModel,
          providers: (providers as any[]).map((p: any) => ({
            name: p.id,
            type: p.id,
            base_url: p?.config?.baseURL || '',
            api_key: p?.config?.apiKey || '',
            models: (p.models || []).map((m: any) => ({ id: m.id, enabled: !!m.enabled }))
          }))
        };
        await llmGatewayApi.importChatConfig(payload as any);
      } catch {}
  ...
  const handleSyncGateway = async () => {
    try {
      ...
      await llmGatewayApi.importChatConfig(payload as any);
      toast.success('已同步到统一模型网关');
    } catch (e) {
      toast.error('同步失败，请检查 9050 服务');
    } finally {
      setSyncing(false);
    }
  };
  ...
  const setAsDefaultChat = async (modelId: string) => {
    try {
      // 若该模型未启用，则先启用
      if (!models.find(m => m.id === modelId)?.enabled) {
        await toggleModel(modelId);
      }
      const resp = await llmGatewayApi.setDefaults({ default_model: modelId });
      setDefaultChatModel(resp?.default_model || modelId);
      setDefaultChatInfo(resp?.default_model_info || null);
      toast.success('已设为默认模型');
    } catch (err: any) {
      // 透出服务端返回，帮助排查为什么“点击设置默认模型报错且后端无日志”
      const msg = err?.response?.data ? JSON.stringify(err.response.data) : (err?.message || '未知错误');
      // eslint-disable-next-line no-console
      console.error('设置默认模型失败(setDefaults):', err?.response?.status, msg);
      toast.error(`设置默认模型失败: ${msg}`);
    }
  };
  const unsetDefaultChat = async () => {
    try {
      const resp = await llmGatewayApi.setDefaults({ default_model: '' });
      setDefaultChatModel(resp?.default_model || '');
      setDefaultChatInfo(resp?.default_model_info || null);
      toast.success('已取消默认模型');
    } catch (err: any) {
      const msg = err?.response?.data ? JSON.stringify(err.response.data) : (err?.message || '未知错误');
      // eslint-disable-next-line no-console
      console.error('取消默认模型失败(setDefaults):', err?.response?.status, msg);
      toast.error(`取消默认模型失败: ${msg}`);
    }
  };
          ...
          <div className="w-full space-y-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                {/* 右侧顶部：默认Chat模型标签，含厂商图标 */}
                <div className="text-sm text-default-500 flex items-center gap-2">
                  <span className="inline-flex items-center gap-2 h-8 px-3 rounded-md border border-default-200 bg-default-100 whitespace-nowrap" title="当前默认Chat模型">
                    {/* 厂商图标（若可得） */}
                    <ProviderIcon
                      providerId={(defaultChatInfo?.provider?.type || defaultChatInfo?.provider?.name || inferredDefaultProviderId || '').toLowerCase()}
                      name={defaultChatInfo?.provider?.name || defaultChatInfo?.provider?.type || inferredDefaultProviderId || ''}
                      className="w-4 h-4"
                    />
                    {/* 默认标记小圆点 */}
                    <span className="w-2 h-2 rounded-full bg-success inline-block" />
                    <span className="text-xs text-default-600">默认</span>
                    <span className="text-xs font-medium text-foreground max-w-[260px] truncate">{defaultChatModel || '-'}</span>
                  </span>
                  <Button
                    size="sm"
                    variant="light"
                    ...
                    onPress={async () => {
                      try {
                        ...
                        setDefaultChatModel(d?.default_model || '');
                        setDefaultChatInfo(d?.default_model_info || null);
                        toast.success('已从网关刷新默认模型');
                      } catch (err: any) {
                        const msg = err?.response?.data ? JSON.stringify(err.response.data) : (err?.message || '未知错误');
                        // eslint-disable-next-line no-console
                        console.error('刷新默认模型失败(getDefaults):', err?.response?.status, msg);
                        toast.error(`刷新默认模型失败: ${msg}`);
                      }
                    }}
                      ...
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          {defaultChatModel === model.id ? (
                            <Button size="sm" color="warning" variant="flat" onPress={unsetDefaultChat}>取消设置</Button>
                          ) : (
                            <Button size="sm" variant="flat" onPress={() => setAsDefaultChat(model.id)}>设为默认</Button>
                          )}
                          {model.isCustom && (
                            <Button
                              size="sm"
