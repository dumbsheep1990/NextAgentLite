...
export function GatewayManager() {
  ...
  const sectionQuery = React.useMemo(() => new URLSearchParams(location.search).get('section'), [location.search]);
  const sectionFromPath = React.useMemo(() => {
    const p = location.pathname || '';
    if (p.includes('/mcp-tools')) return 'mcp';
    if (p.includes('/api-tools')) return 'api';
    return '';
  }, [location.pathname]);
  const sectionParam = sectionQuery || sectionFromPath;
  const showToolsOnly = sectionParam === 'mcp';
  const showApiOnly = sectionParam === 'api';
  const showAll = !sectionParam;
        ...
        {isLoading ? (
          ...
          <Table aria-label={t('gateway.table_view')}>
            <TableHeader>
              ...
              <TableColumn width="25%">API</TableColumn>
      ...
      >
        <ModalContent>
          {() => (
            <>
              <ModalHeader className="flex flex-col gap-1">
                {currentModalServer?.name} - API
              </ModalHeader>
              <ModalBody>
                {currentModalServer?.servers && currentModalServer.servers.length > 0 && (
                  <div className="space-y-6">
                    <div>
                      <h4 className="text-sm font-semibold mb-2">已启用 API</h4>
                      <div className="flex flex-wrap gap-1">
                    ...
                    <div className="mt-4 pt-4 border-t">
                      <h4 className="text-sm font-semibold mb-2">全部 API</h4>
                      <div className="flex flex-wrap gap-1">
