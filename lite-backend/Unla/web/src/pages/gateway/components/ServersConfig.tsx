            ...
            <div className="space-y-4">
              ...
              <div className="bg-content1 p-4 rounded-medium border border-content2">
                <div className="flex justify-between items-center mb-3">
                  <h4 className="text-md font-medium">允许的 API</h4>
                </div>
                ...
                <div className="mt-2">
                  <h4 className="text-sm font-medium mb-2">添加 API</h4>
                  <div className="flex flex-wrap gap-2">
                    ...
                    {(parsedConfig?.tools || []).length > 0 &&
                     (parsedConfig?.tools || []).every((tool: { name?: string }) => tool.name && (server.allowedTools || []).includes(tool.name)) && (
                      <span className="text-sm text-default-500">已全部添加 API</span>
                    )}
                    {(parsedConfig?.tools || []).length === 0 && (
                      <span className="text-sm text-default-500">暂无可用 API</span>
                    )}
                  </div>
                </div>
              </div>
