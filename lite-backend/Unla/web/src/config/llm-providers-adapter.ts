...
const getDefaultCheckModel = (providerId: string): string => {
  const checkModels: Record<string, string> = {
    ...
    'xai': 'grok-beta',
    // SiliconCloud (SiliconFlow): use a widely-available open model id
    // If your account has access to a different model, update in UI
    'siliconcloud': 'Qwen/Qwen2.5-7B-Instruct'
  };
