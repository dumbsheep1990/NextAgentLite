...
import re
import numpy as np
# Backward-compat alias for packages still referencing np.infty on NumPy>=2.0
if not hasattr(np, "infty"):
    np.infty = np.inf
import tiktoken
# UMAP/pyNNDescent may fail under incompatible NumPy/Numba stacks.
# Fall back gracefully when import errors occur.
try:
    import umap  # type: ignore
except Exception as _umap_err:  # noqa: BLE001
    umap = None  # type: ignore
    logging.warning(
        "UMAP unavailable or incompatible; falling back to no-reduction clustering: %s",
        _umap_err,
    )
import copy
import asyncio
...
def global_cluster_embeddings(
    ...
    if umap is None:
        # Fallback: no dimensionality reduction
        return embeddings
