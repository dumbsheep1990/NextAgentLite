...
<script lang="ts">
  ...
  let searchModeOptions = $derived([
    ...(allowFullTextSearch ? [{ label: "全文搜索", value: "full-text" }] : []),
    ...(allowVectorSearch ? [{ label: "向量搜索", value: "vector" }] : []),
    ...(allowNearestNeighborSearch ? [{ label: "最近邻", value: "neighbors" }] : []),
  ]);
    ...
    <div class="m-2 flex flex-row justify-between items-center">
      <div class="flex flex-row flex-1 justify-between">
        <div class="flex flex-row items-center">
          <div class="flex-1">
            {#if searcher}
              <div class="relative">
                <Input type="search" placeholder="搜索..." className="w-64" bind:value={searchQuery} />
            ...
            {:else}
              <div class="text-slate-500 dark:text-slate-400">向量Atlas可视化</div>
            {/if}
          </div>
        </div>
          ...
          {#if showEmbedding}
            ...
            {#if embeddingViewMode == "density"}
              <div class="select-none flex items-center gap-2">
                <span class="text-slate-500 dark:text-slate-400">阈值</span>
      ...
      <div class="flex flex-row gap-0.5">
        <PopupButton icon={IconSettings} title="选项">
          <div class="min-w-96">
            <!-- Tooltip settings -->
            <h4 class="text-slate-500 dark:text-slate-400 mb-2 select-none">工具提示</h4>
            <Select
              class="w-full"
            ...
            {#if columns.length > 0}
              <!-- Text style settings -->
              <h4 class="text-slate-500 dark:text-slate-400 my-2 select-none">文本样式</h4>
              <TextStylePicker
                columns={columns}
            ...
            {/if}
            <!-- Export -->
            <h4 class="text-slate-500 dark:text-slate-400 my-2 select-none">导出</h4>
            ...
            <h4 class="text-slate-500 dark:text-slate-400 my-2 select-none">关于</h4>
            <div>向量Atlas可视化, {EMBEDDING_ATLAS_VERSION}</div>
          </div>
        </PopupButton>
        {#if colorScheme == null}
          <Button
        ...
        {/if}
        {#if projectionColumns != null}
          <ToggleButton icon={IconEmbeddingView} title="显示/隐藏向量图" bind:checked={showEmbedding} />
        {/if}
        <ToggleButton icon={IconTable} title="显示/隐藏表格" bind:checked={showTable} />
        <ToggleButton icon={IconMenu} title="显示/隐藏侧边栏" bind:checked={showSidebar} />
      </div>
    </div>
