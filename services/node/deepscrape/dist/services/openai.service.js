...
class OpenAIService {
    ...
    async getCompletion(messages, options = {}, responseFormat) {
        try {
            const { temperature = 0.2, maxTokens = 4000, topP, frequencyPenalty, presencePenalty, stop, seed, user } = options;
            ...
            try {
                ...
                logger_1.logger.warn(`Failed to parse JSON response: ${parseError instanceof Error ? parseError.message : String(parseError)}`);
                return {
                    success: true,
                    data: content,
                    metadata: {
