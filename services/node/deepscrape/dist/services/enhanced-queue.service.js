...
exports.EnhancedQueueService = void 0;
...
class EnhancedQueueService {
    constructor(queueName, config = {}) {
        ...
        this.redis = redis_service_1.redisClient;
        ...
        this.queueEvents = new bullmq_1.QueueEvents(queueName, { connection });
        this.setupEventListeners();
        // Start dynamic scaling if enabled
        if (this.config.enableDynamicScaling) {
            this.startDynamicScaling();
        }
        logger_1.logger.info(`Enhanced queue service initialized: ${queueName}`, {
            config: this.config,
            queueName
        });
    }
    ...
    async addJob(jobName, data, options = {}) {
        ...
        try {
            ...
            logger_1.logger.debug(`Job added to queue: ${jobName}`, {
                jobId: job.id,
            ...
            logger_1.logger.error(`Failed to add job to queue: ${jobName}`, {
                error: error.message,
                data,
                options: jobOptions
            });
    ...
    async addBulkJobs(jobs) {
        ...
        try {
            ...
            logger_1.logger.info(`Bulk jobs added to queue`, {
            ...
            logger_1.logger.error(`Failed to add bulk jobs to queue`, {
                error: error.message,
     ...
     */
    initializeWorker(processor) {
        if (this.worker) {
            logger_1.logger.warn('Worker already initialized, stopping existing worker');
            this.worker.close();
        }
        this.worker = new bullmq_1.Worker(this.queue.name, async (job) => {
            ...
            try {
                ...
                logger_1.logger.info(`Processing job: ${job.name}`, {
                    jobId,
                ...
                logger_1.logger.info(`Job completed: ${job.name}`, {
                    jobId,
                    duration,
                    attempt: job.attemptsMade + 1
                });
                ...
                this.cleanupLockExtension(jobId);
                const duration = Date.now() - startTime;
                logger_1.logger.error(`Job failed: ${job.name}`, {
                    jobId,
                    duration,
                    attempt: job.attemptsMade + 1,
                    error: error.message
        ...
        logger_1.logger.info(`Worker initialized with concurrency: ${this.config.concurrency}`);
        return this.worker;
    }
    /**
     * Setup automatic lock extension for long-running jobs
     */
    setupLockExtension(job) {
        const jobId = job.id;
        const interval = setInterval(async () => {
            try {
                await job.extendLock('', this.config.lockRenewTime);
                logger_1.logger.debug(`Extended lock for job ${jobId}`);
            }
            catch (error) {
                logger_1.logger.warn(`Failed to extend lock for job ${jobId}`, {
                    error: error.message
                });
    ...
    async adjustConcurrency() {
        ...
        try {
            ...
            if (newConcurrency !== this.config.concurrency) {
                ...
                logger_1.logger.info(`Adjusting queue concurrency to ${newConcurrency}`, {
            ...
            logger_1.logger.error('Failed to adjust concurrency', {
                error: error.message
            });
        }
    }
    ...
    setupEventListeners() {
        this.queueEvents.on('completed', ({ jobId, returnvalue }) => {
            logger_1.logger.debug('Job completed', { jobId, hasReturnValue: !!returnvalue });
        });
        this.queueEvents.on('failed', ({ jobId, failedReason }) => {
            logger_1.logger.error('Job failed', { jobId, failedReason });
        });
        ...
        this.queueEvents.on('removed', ({ jobId }) => {
            logger_1.logger.debug('Job removed', { jobId });
        });
    }
    /**
    ...
    setupWorkerEventListeners() {
        ...
        this.worker.on('completed', (job) => {
            logger_1.logger.debug(`Worker completed job: ${job.name}`, { jobId: job.id });
        });
        this.worker.on('failed', (job, err) => {
            logger_1.logger.error(`Worker failed job: ${job?.name}`, {
                jobId: job?.id,
                error: err.message
            });
        });
    ...
    async cleanup(maxAge = 24 * 60 * 60 * 1000) {
        ...
        try {
            ...
            logger_1.logger.info('Queue cleanup completed', { maxAge });
        }
        catch (error) {
            logger_1.logger.error('Queue cleanup failed', { error: error.message });
        }
    }
    /**
     * Graceful shutdown
    ...
    async close() {
        ...
        if (this.worker) {
            ...
            logger_1.logger.info('Worker closed');
        }
        ...
        logger_1.logger.info('Enhanced queue service shutdown complete');
    }
...
exports.EnhancedQueueService = EnhancedQueueService;
