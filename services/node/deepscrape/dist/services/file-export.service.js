...
class FileExportService {
    ...
    generateFilename(url, crawlId) {
        try {
            ...
            logger_1.logger.warn('Failed to parse URL for filename generation', { url, error: error instanceof Error ? error.message : String(error) });
            const timestamp = this.formatTimestampForFilename(new Date().toISOString());
    ...
    parseFrontmatterMetadata(content) {
        ...
        try {
            ...
            logger_1.logger.debug('Failed to parse frontmatter', { error: e instanceof Error ? e.message : String(e) });
        }
        return metadata;
    }
    /**
