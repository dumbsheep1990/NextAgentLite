...
class ContentCleaner {
    ...
    findMainContentBySelectors($) {
        const mainContentSelectors = [
            // 中国政府网站特殊选择器 - 优先级最高
            '#Zoom', '.DocHtmlCon', '.TRS_PreAppend', '.zwgk_content', '.doc_content',
            // 标准内容选择器
            ...
            '#document-content', '.page-content', '.body-content',
            // body作为最后兜底选项
            'body', '.body'
        ];
        ...
        for (const selector of mainContentSelectors) {
            ...
            if (selector === '#Zoom' || selector === '.DocHtmlCon') {
                logger_1.logger.info(`[Gov Selector] ${selector}: exists=${exists}, valid=${isValid}, textLen=${exists ? element.text().trim().length : 0}`);
            }
            ...
            if (this.isValidContentElement(element)) {
                ...
                // 政府网站特殊处理：即使没有标题，如果有足够的段落和文本，也认为是有效内容
                const isGovernmentSelector = ['#Zoom', '.DocHtmlCon', '.TRS_PreAppend', '.zwgk_content', '.doc_content'].includes(selector);
                const hasRichContent = textLength > 2000 && paragraphCount > 20;

                if (isGovernmentSelector) {
                    logger_1.logger.info(`  [Gov Selector Details] ${selector}: headings=${foundHeadings}, text=${textLength}, paragraphs=${paragraphCount}, hasRich=${hasRichContent}`);
                }

                if (foundHeadings > 0 || (isGovernmentSelector && hasRichContent)) {
                    mainContent = element;
                    logger_1.logger.info(`Found main content using selector: ${selector} with ${foundHeadings} headings, ${textLength} chars, ${paragraphCount} paragraphs`);
                    break;
                }
                else if (!mainContent && textLength > 500) {
                    mainContent = element;
